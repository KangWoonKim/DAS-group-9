...
```
@tbl-obe_year indicates that the lowest value recorded was in 2009, with the peak being in 2010 (at 30.5%) and the rate decreasing after this year. 
...
kable(x=summary(model_factors)$coefficients[,c(1,4)], col.names = c("Estimate","p - values"))

```

from the `model_factors` we observe that `SexMale` and `FruitYes` are not significant parameters since the p-values are greater than 0.05 . 
So, we use stepAIC function to find a better model.
...
#| label: tbl-regmodel2
#| tbl-cap: Estimates of the regression model coefficients.
kable(x=summary(AIC)$coefficients[,c(1,4)], col.names = c("Estimate","p - values"))
...
However, we are also interested in producing a 95% confidence interval for these log-odds. This is done follow @tbl-confidenceinterval

we could check this by visually (@fig-log_odds) seeing that confidence interval for the each factors of the log-odds.
...
log_odds_ci <- data.frame(
  groups = c( "Age","Education Higher","Education HNC/D", "Education no", "Education other", "Education Standard", "Vegetarian"),
...
kable(log_odds_ci, col.names = c("Groups", "confidence interval low boundary", "confidence interval uppter boundary"), caption = "Confidence interval of the log odds", format="pipe")
```
...
predict(model_factors, type= "response")
```
