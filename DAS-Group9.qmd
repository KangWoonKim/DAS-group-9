...
In @sec-eda, we will study the relationship between obesity and various influencing factors, including age, gender, education level, and daily consumption of vegetables and fruits in order to uncover preliminary insights and trends within the data through exploratory data analysis techniques. This is done using a variety of plots and tables that emphasise the relationship between the prevalence of obesity in Scotland and the various influencing factors.
...
To evaluate the goodness of fit for the logistic regression model, the stepwise selection procedure known as stepAIC was employed. This methodological approach applies both forward selection and backward elimination techniques to optimize the model based on the Akaike Information Criterion (AIC). The stepAIC process iteratively adds and removes variables, assessing the impact of each variable on the AIC value. A lower AIC value generally indicates a better-fitting model, provided that it maintains a balance between goodness of fit and model simplicity.

The application of stepAIC to the logistic regression model resulted in the exclusion of variables 'Sex' and 'Fruit' intake, as their presence did not contribute to a reduction in the AIC score, suggesting that their inclusion did not improve the model's predictive ability significantly. The final model, which excludes these variables, is considered to be more parsimonious and theoretically sound for predicting the probability of obesity in the studied population.
...
The logistic regression equation derived from the model refinement using the stepAIC process, each independent variable's coefficient is integral to the calculation of the log-odds of the probability,
p, of an individual being obese. The logistic function is represented as follows::
$$
...
where p is probability of being obese and 1-p is probability of being not obese.
The model delineates that, there is a 1.2% escalation in the log-odds of obesity, affirming age as a significant factor, with other variables held constant.

The educational background introduces a discernible stratification in obesity prediction. Individuals with higher education levels exhibit a 14.8% increase in the log-odds of obesity, whereas those with an HNC/D level present a 24% rise. The absence of education correlates with a pronounced 38.6% surge in the log-odds, whereas other unspecified educational backgrounds are associated with a 21.9% increase. Similarly, possessing standard grade education corresponds to a 30.5% increase in the log-odds of obesity.

Interestingly, the negative coefficient for vegetable intake, indicates that consuming vegetables is associated with a decrease in the log-odds of being obese by 14%. This suggests that a diet including vegetables is inversely related to the likelihood of being obese.

Further analysis and interpretation of this model should take into account the log-odds 95% confidence intervals for these coefficients to assess the precision of the estimates. These intervals are essential for understanding the range within which the true value of the coefficients is likely to lie, with a 95% level of confidence. (see @tbl-the_log_odds)
 
The confidence intervals give a range of plausible values for the log-odds and odds ratios, with a 95% level of confidence. Narrow confidence intervals, as seen with age and vegetable intake, suggest a more precise estimate, while wider intervals suggest greater uncertainty. In our data, all intervals do not cross 0 for log-odds ratios, which indicates that all predictors are statistically significant at the 95% confidence level.
